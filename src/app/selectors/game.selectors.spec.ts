...
describe('Selector upgradesPossible', () => {
  it('should return false for non-affordable properties', () => {
    expect(
      upgradesPossible.projector({
        funds: 0,
        properties: {
          salary: { level: 3, upgradeConditions: {} },
          education: { level: 2, upgradeConditions: {} },
        },
      })
  ...
  it('should return a single possible upgrade', () => {
    expect(
      upgradesPossible.projector({
        funds: 5,
        properties: { salary: { level: 3, upgradeConditions: {} } },
      })
    ).toEqual({ salary: true });
  });
  ...
  it('should return false for properties where an upgrade condition is not met', () => {
    expect(
      upgradesPossible.projector({
        funds: 100,
        properties: {
          salary: { level: 1, upgradeConditions: {
            education: 1
          } },
          education: { level: 0, upgradeConditions: {} },
        },
      })
